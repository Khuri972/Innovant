...
public class MyDynamicCalendar extends LinearLayout {
    public static final int ONE_DAY_EVENT = 1, BIRTHDAY_EVENT = 5, HOLYDAY_EVENT = 6, MULTIPLE_DAY_EVENT = 7;
    public static final int MULTIPLEDAY_EVENT_LEFT = 2, MULTIPLEDAY_EVENT_RIGHT = 3, MULTIPLEDAY_EVENT_CENTER = 4;
...
//        }
        ...
        recyclerView_dates.setNestedScrollingEnabled(false);
        recyclerView_hours.setNestedScrollingEnabled(false);
        ...
        iv_next.setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View v) {
                if (AppConstants.isShowMonth) {
                    ...
                    setDayView("add", AppConstants.main_calendar);
        ...
        iv_previous.setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View v) {
                if (AppConstants.isShowMonth) {
                    ...
                    setDayView("sub", AppConstants.main_calendar);
                } else if (AppConstants.isAgenda) {
                    setAgendaView("sub");
                }
            }
...
//            }
    ...
    public void addHoliday(String eventId, int event_type, String date, String name, int image) {
        if (!TextUtils.isEmpty(date) && !TextUtils.isEmpty(name)) {
            String tmpDate = GlobalMethods.convertDate(date, AppConstants.sdfDate, AppConstants.sdfDate);
            AppConstants.eventList.add(new EventModel(eventId, event_type, tmpDate, name, image));
        }
    }

    public void addBirthday(String eventId, int event_type, String date, String name, int image) {
        if (!TextUtils.isEmpty(date) && !TextUtils.isEmpty(name)) {
            String tmpDate = GlobalMethods.convertDate(date, AppConstants.sdfDate, AppConstants.sdfDate);
            AppConstants.eventList.add(new EventModel(eventId, event_type, tmpDate, name, image));
        }
    ...
    public void addEvent(String eventId, int event_type, String date, String startTime, String endTime, String name) {
        if (!TextUtils.isEmpty(date) && !TextUtils.isEmpty(startTime) && !TextUtils.isEmpty(endTime) && !TextUtils.isEmpty(name)) {
            ...
            AppConstants.eventList.add(new EventModel(eventId, event_type, tmpDate, tmpStartTime, tmpEndTime, name));
        }
    }

    public void addEvent(String eventId, String date, String startTime, String endTime, String name, int image) {
        if (!TextUtils.isEmpty(date) && !TextUtils.isEmpty(startTime) && !TextUtils.isEmpty(endTime) && !TextUtils.isEmpty(name)) {
            String tmpDate = GlobalMethods.convertDate(date, AppConstants.sdfDate, AppConstants.sdfDate);
            String tmpStartTime = GlobalMethods.convertDate(startTime, AppConstants.sdfHourMinute, AppConstants.sdfHourMinute);
            String tmpEndTime = GlobalMethods.convertDate(endTime, AppConstants.sdfHourMinute, AppConstants.sdfHourMinute);
            AppConstants.eventList.add(new EventModel(eventId, tmpDate, tmpStartTime, tmpEndTime, name, image));
        }
    }
            ...
            public void onClick(Date date) {
                ...
                for (int i = 0; i < AppConstants.eventList.size(); i++) {
                    if (AppConstants.eventList.get(i).getStrDate().equals(AppConstants.sdfDate.format(date))) {
                        eventModelList.add(new EventModel(AppConstants.eventList.get(i).getEventId(),
                                AppConstants.eventList.get(i).getEvent_type(),
...
//        setWeekView("");
    ...
    public void showDayView(Calendar cal) {
        setDayView("", cal);
    }

    private void setDayView(String sign, Calendar cal) {
        AppConstants.main_calendar = cal;
        showEventsModelList = new ArrayList<>();
...
//        calendar.setTime(AppConstants.main_calendar.getTime());
    ...
    public void goToCurrentDate() {
        ...
        if (AppConstants.isShowMonth) {
            ...
            setDayView("", AppConstants.main_calendar);
        } else if (AppConstants.isAgenda) {
            setAgendaView("");
        }
    }
