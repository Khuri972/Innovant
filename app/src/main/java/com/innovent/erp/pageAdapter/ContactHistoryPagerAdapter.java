...
public class ContactHistoryPagerAdapter extends FragmentPagerAdapter {
    Fragment f;
    List<Fragment> mFragmentList = new ArrayList<>();
    ArrayList<GeneralModel> categoryModels = new ArrayList<>();

    public ContactHistoryPagerAdapter(FragmentManager fm, ArrayList<GeneralModel> topCategoryModels) {
        super(fm);
    ...
    public Fragment getItem(int position) {
        if (position == 0) {
            f = new ContactCompanyFragment();
            mFragmentList.add(f);
            return f;
        } else {
            f = new ContactIndividualFragment();
            mFragmentList.add(f);
            return f;
        }
    }
