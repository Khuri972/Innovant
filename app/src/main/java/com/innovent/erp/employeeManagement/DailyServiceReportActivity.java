...
public class DailyServiceReportActivity extends AppCompatActivity implements DeleteDailyReport {
    ...
    ArrayList<DailyServiceReportModel> data = new ArrayList<>();
    DailyServiceReportAdapter adapter;
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        setContentView(R.layout.activity_daily_service_report);
        ButterKnife.bind(this);
        getSupportActionBar().setDisplayHomeAsUpEnabled(true);
        getSupportActionBar().setDefaultDisplayHomeAsUpEnabled(true);
        getSupportActionBar().setTitle("Daily Service Report");
        myPreferences = new MyPreferences(this);

        adapter = new DailyServiceReportAdapter(DailyServiceReportActivity.this, data, "");
        recycleView.setAdapter(adapter);
        recycleView.setLayoutManager(new LinearLayoutManager(DailyServiceReportActivity.this, LinearLayoutManager.VERTICAL, false));

        addReport.setOnClickListener(new View.OnClickListener() {
            @Override
            public void onClick(View view) {
                Intent intent = new Intent(DailyServiceReportActivity.this, AddServiceDailyReport.class);
                intent.putExtra("type", "0");
                startActivityForResult(intent, 0);
            }
        });

        if (GlobalElements.isConnectingToInternet(DailyServiceReportActivity.this)) {
            getDailyServiceReport();
        } else {
            GlobalElements.showDialog(DailyServiceReportActivity.this);
        }
    ...
    protected void onActivityResult(int requestCode, int resultCode, Intent _data) {
        ...
        if (resultCode == 10) {
            try {
                DailyServiceReportModel da = (DailyServiceReportModel) _data.getSerializableExtra("data");
                data.add(0, da);
            ...
            try {
                ...
                String PathHolder = PathUtil.getPath(DailyServiceReportActivity.this, ur);
    ...
    public String getMimeType(Uri uri) {
        ...
        if (uri.getScheme().equals(ContentResolver.SCHEME_CONTENT)) {
            ContentResolver cr = DailyServiceReportActivity.this.getContentResolver();
    ...
    private void getDailyServiceReport() {
        try {
            final ProgressDialog pd = new ProgressDialog(DailyServiceReportActivity.this);
            pd.setTitle("Please Wait");
            pd.setMessage("Loading");
            pd.setCancelable(true);
            pd.show();
            RequestInterface request = RetrofitClient.getClient().create(RequestInterface.class);
            Call<ResponseBody> call = request.getDailyServiceReport(myPreferences.getPreferences(MyPreferences.id));
                    ...
                    try {
                        ...
                        if (json.getInt("ack") == 1) {
                            ...
                            for (int i = 0; i < result.length(); i++) {
                                ...
                                DailyServiceReportModel da = new DailyServiceReportModel();
                                da.setId("" + c.getString("id"));
                                da.setAddress("" + c.getString("address"));
                                da.setCustomer_name("" + c.getString("customer_name"));
                                da.setPurpose("" + c.getString("service_type_id"));
                                da.setPurpose_slug("" + c.getString("service_type_name"));
