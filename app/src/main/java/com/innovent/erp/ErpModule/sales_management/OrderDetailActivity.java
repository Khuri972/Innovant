...
public class OrderDetailActivity extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        try {
            ...
            model = (SalesOrderModel) bundle.getSerializable("data");
            setData();
            getSupportActionBar().setTitle("" + model.getOrder_no());
        } catch (Exception e) {
            e.printStackTrace();
            getSupportActionBar().setTitle("Quotation Detail");
        }

        if (GlobalElements.isConnectingToInternet(OrderDetailActivity.this)) {
            getQuatationRequestDetail();
        } else {
            GlobalElements.showDialog(OrderDetailActivity.this);
        }
    }
    ...
    protected void onActivityResult(int requestCode, int resultCode, Intent _data) {
        ...
        try {
            ...
            if (resultCode == 11) {
                model = (SalesOrderModel) _data.getSerializableExtra("model");
                setData();
                data.clear();
                data = (ArrayList<ProductModel>) _data.getSerializableExtra("data");
                adapter = new QuatationRequestDisplayProductAdapter(OrderDetailActivity.this, data);
    ...
    public boolean onOptionsItemSelected(MenuItem item) {
        switch (item.getItemId()) {
            ...
            case R.id.action_update:
                try {
                    Intent intent = new Intent(OrderDetailActivity.this, AddSalesOrder.class);
                    Bundle bundle = new Bundle();
                    bundle.putSerializable("order_data", model);
    ...
    private void getQuatationRequestDetail() {
        ...
        Call<ResponseBody> call = req.getOrderDetail(model.getId(), myPreferences.getPreferences(MyPreferences.id));
            ...
            public void onResponse(Call<ResponseBody> call, Response<ResponseBody> response) {
                try {
                    ...
                    if (json.getInt("ack") == 1) {
                        ...
                        recycleview.setAdapter(adapter);
                        recycleview.setLayoutManager(new LinearLayoutManager(OrderDetailActivity.this, LinearLayoutManager.VERTICAL, false));
                        Toaster.show(OrderDetailActivity.this, "" + json.getString("ack_msg"), false, Toaster.DANGER);
                    }
                } catch (Exception e) {
                    e.printStackTrace();
                    pd.dismiss();
